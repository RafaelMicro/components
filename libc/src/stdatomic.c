...
#define CMP_EXCHANGE(n, type)                                                  \
    bool __atomic_compare_exchange_##n(type* mem, type* expect, type desired,  \
...
#define SYNC_FETCH_OP(op, n, type)                                             \
    type CLANG_ATOMIC_SUFFIX(__sync_fetch_and_##op##_##n)(type * ptr,          \
                                                          type value) {        \
...
#define SYNC_BOOL_CMP_EXCHANGE(n, type)                                        \
    bool CLANG_ATOMIC_SUFFIX(__sync_bool_compare_and_swap_##n)(                \
...
#define SYNC_VAL_CMP_EXCHANGE(n, type)                                         \
    type CLANG_ATOMIC_SUFFIX(__sync_val_compare_and_swap_##n)(                 \
        type * ptr, type oldval, type newval) {                                \
        unsigned state = _ATOMIC_ENTER_CRITICAL();                             \
        type ret = *ptr;                                                       \
        if (*ptr == oldval) {                                                  \

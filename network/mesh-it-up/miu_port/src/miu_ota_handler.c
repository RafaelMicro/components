...
const char* OtaStateToString(ota_state_t state) {
    static const char* const kOtaStateStrings[] = {
        "Idle",           "DataSending", "DataReceiving",
...
void ota_bootloader_info_check() {
    ...
    do {
        ...
        while (flash_check_busy())
            ;
        flash_write_page((uint32_t)&t_bootloader_ota_info,
            ...
            if (NULL != buf) {
                ...
                if (otCoapMessageGetType(aMessage)
                    ...
                    error = otCoapSendResponseWithParameters(
                        instance, responseMessage, aMessageInfo, NULL);
                    if (error != OT_ERROR_NONE) {
                        break;
                    }
                }
...
static void ota_coap_report_proccess(void* aContext, otMessage* aMessage,
                                     const otMessageInfo* aMessageInfo) {
    char string[OT_IP6_ADDRESS_STRING_SIZE];
    uint8_t buf[16];
    ...
    if (length > 0) {
        ...
        while (length > 0) {
            ...
            otMessageRead(aMessage, otMessageGetOffset(aMessage) + bytesPrinted,
                          buf, bytesToPrint);
        ...
        if (instance) {
            ...
            if ((coapType == OT_COAP_TYPE_CONFIRMABLE)
                ...
                error = otCoapSendRequestWithParameters(
                    instance, message, &messageInfo, &ota_coap_ack_process,
                    NULL, NULL);
            } else {
                error = otCoapSendRequestWithParameters(
                    instance, message, &messageInfo, NULL, NULL, NULL);
            }
        }
    } while (0);
...
static void ota_data_send_handler() {
    ...
    do {
        ...
        if (payload) {
            ...
            ota_printf("\r [s] ota data: [%3d%%] [%u/%u]",
                       ((ota_data.seq * 100) / (g_ota_toatol_num - 1)),
                       ota_data.seq, (g_ota_toatol_num - 1));
            if (is_unicst == true) {
                have_data_ack = false;
            } else {
                g_ota_start_index++;
    ...
    do {
        ...
        if (payload) {
            ...
            ota_printf("[s] ota response %u %u %x \n", random_index,
                       ota_response.seq, (uint32_t)temp_addr);
...
static void ota_timer_handler() {
    ...
    switch (g_ota_state) {
        ...
        case OTA_UNICASE_RECEIVING: ota_unicast_received_handler(); break;
...
static int _cli_cmd_ota(int argc, char** argv, cb_shell_out_t log_out,
                        void* pExtra) {
    if (argc > 2) {

...
#define OTRADIO_MAX_PSDU                    OT_RADIO_FRAME_MAX_SIZE + 28
...
otRadioCaps otPlatRadioGetCaps(otInstance* aInstance) {
    ...
    return (
        otRadioCaps)(OT_RADIO_CAPS_ACK_TIMEOUT | OT_RADIO_CAPS_CSMA_BACKOFF |
#if OPENTHREAD_CONFIG_THREAD_VERSION >= OT_THREAD_VERSION_1_2
...
otError otPlatRadioSetCcaEnergyDetectThreshold(otInstance* aInstance,
    ...
    lmac15p4_phy_pib_set(sTurnaroundTime, sCCAMode, sCCAThreshold,
                         sCCADuration);
...
void otPlatRadioSetExtendedAddress(otInstance* aInstance,
    ...
    sExtendAddr_0 = (aAddress->m8[0] | (aAddress->m8[1] << 8)
                     | (aAddress->m8[2] << 16) | (aAddress->m8[3] << 24));
    sExtendAddr_1 = (aAddress->m8[4] | (aAddress->m8[5] << 8)
                     | (aAddress->m8[6] << 16) | (aAddress->m8[7] << 24));

    lmac15p4_address_filter_set(0, sPromiscuous, sShortAddress, sExtendAddr_0,
...
otError otPlatRadioEnergyScan(otInstance* aInstance, uint8_t aScanChannel,
    ...
    subg_ctrl_frequency_set(FREQ
                            + (CHANNEL_SPACING * (aScanChannel - kMinChannel)));
...
otError otPlatRadioReceive(otInstance* aInstance, uint8_t aChannel) {
    ...
    subg_ctrl_frequency_set(FREQ
                            + (CHANNEL_SPACING * (aChannel - kMinChannel)));
...
static uint16_t getCslPhase(void) {
    ...
    uint32_t diff = ((sCslSampleTime % cslPeriodInUs)
                     - (curTime % cslPeriodInUs) + cslPeriodInUs)
                    % cslPeriodInUs;
...
otError otPlatRadioTransmit(otInstance* aInstance, otRadioFrame* aFrame) {
    ...
    static uint8_t tx_data[OT_RADIO_FRAME_MAX_SIZE + 4];

    otError error = OT_ERROR_INVALID_STATE;
    if (otRadio_var.pTxFrame == NULL) {
        otRadio_var.pTxFrame = aFrame;
        sCurrentChannel = aFrame->mChannel;
        subg_ctrl_frequency_set(
            FREQ + (CHANNEL_SPACING * (aFrame->mChannel - kMinChannel)));
    }
#if OPENTHREAD_CONFIG_MAC_CSL_RECEIVER_ENABLE
    ...
    error = radioProcessTransmitSecurity(aFrame);
    ...
    tx_data[0] = ((sMacFrameCounter >> 0) & 0xFF);
    tx_data[1] = ((sMacFrameCounter >> 8) & 0xFF);
    tx_data[2] = ((sMacFrameCounter >> 16) & 0xFF);
    tx_data[3] = ((sMacFrameCounter >> 24) & 0xFF);

    memcpy((tx_data + 4), aFrame->mPsdu, aFrame->mLength);

    if (aFrame->mInfo.mTxInfo.mTxDelay != 0) {
        tx_control ^= (1 << 1);
    }
    // log_info("tx seq %d", otMacFrameGetSequence(aFrame));
    if (lmac15p4_tx_data_send(0, tx_data, ((aFrame->mLength + 4) - 2),
                              tx_control, otMacFrameGetSequence(aFrame))
        != 0) {
        log_error("Tx failed !");
...
#ifdef CONFIG_MIU_DEVICE_TYPE_RCP
    return 255;
#else
    return 175;
#endif
...
void otPlatRadioSetMacKey(otInstance* aInstance, uint8_t aKeyIdMode,
                          uint8_t aKeyId, const otMacKeyMaterial* aPrevKey,
...
otError otPlatRadioConfigureEnhAckProbing(otInstance* aInstance,
    ...
    OT_UNUSED_VARIABLE(aInstance);
    if ((aLinkMetrics.mLqi + aLinkMetrics.mLinkMargin + aLinkMetrics.mRssi)
        < 3) {
...
static bool hasFramePending(const otRadioFrame* aFrame) {
    ...
    switch (src.mType) {
        case OT_MAC_ADDRESS_TYPE_SHORT:
            rval = utilsSoftSrcMatchShortFindEntry(src.mAddress.mShortAddress)
                   >= 0;
            break;
...
void ot_radioTask(ot_system_event_t trxEvent) {
    ...
    if (otRadio_var.pTxFrame) {
        if ((OT_SYSTEM_EVENT_RADIO_TX_ALL_MASK & trxEvent)) {
            ...
            otRadio_var.tstx = 0;
            OT_EXIT_CRITICAL();
            // log_info("ot tx seq %d done", otMacFrameGetSequence(txframe));
    ...
    if (trxEvent & OT_SYSTEM_EVENT_RADIO_RX_DONE) {
        ...
        if (pframe) {
            // log_info("=== Radio Status ===\n");
            // log_info("dbgRxFrameNum: %u\n", otRadio_var.dbgRxFrameNum);
            // log_info("dbgFrameNum: %u\n", otRadio_var.dbgFrameNum);
            // log_info("dbgMaxPendingFrameNum: %u\n", otRadio_var.dbgMaxPendingFrameNum);
            // log_info("=====================\n");
            // log_info("ot rx seq %d done",
            //          otMacFrameGetSequence(&pframe->frame));
        ...
        log_warn("Radio RX Buffer Full! Drop frame.");
...
static void _TxDoneEvent(uint32_t tx_status) {
    ...
    if (otRadio_var.pTxFrame) {
        ...
        if (0 == tx_status) {
            ...
            otRadio_var.pAckFrame->mLength = lmac15p4_read_ack(
                (uint8_t*)otRadio_var.pAckFrame->mPsdu,
                (uint8_t*)&ack_now32Time, 1);
            otRadio_var.pAckFrame->mInfo.mRxInfo.mTimestamp =
                longtime_to_longlong_time(&ack_prev32Time, ack_now32Time,
                                          &ack_timerWraps);
            // log_info("tx seq %d done", otMacFrameGetSequence(txframe));
...
static void _RxDoneEvent(uint16_t packet_length, uint8_t* rx_data_address,
    ...
    if (crc_status == 0) {
        if (otRadio_var.dbgRxFrameNum >= OTRADIO_RX_FRAME_BUFFER_NUM) {
            ...
            if (p) {
                ...
                p->frame.mInfo.mRxInfo.mTimestamp = longtime_to_longlong_time(
                    &rx_prev32Time, rx_now32Time, &rx_timerWraps);
#ifdef CONFIG_MIU_DEVICE_TYPE_RCP
                p->frame.mInfo.mRxInfo.mTimestamp -= (2500);
#else
                p->frame.mInfo.mRxInfo.mTimestamp -= (2030 * 10);
#endif
                p->frame.mLength = (packet_length - 10);
                if (OTRADIO_MAX_PSDU < p->frame.mLength) {
                    log_warn("Rx Done len error %d", p->frame.mLength);
                    return;
                }
                memcpy(p->frame.mPsdu, (rx_data_address + 9), p->frame.mLength);

                p->frame.mChannel = sCurrentChannel;
                rssi = rssi >= RAFAEL_RECEIVE_SENSITIVITY
                           ? RAFAEL_RECEIVE_SENSITIVITY
                           : rssi;
                p->frame.mInfo.mRxInfo.mRssi = -rssi;
                p->frame.mInfo.mRxInfo.mLqi =
                    ((RAFAEL_RECEIVE_SENSITIVITY - rssi) * 0xFF)
                    / RAFAEL_RECEIVE_SENSITIVITY;
                ...
                if (
#if OPENTHREAD_CONFIG_THREAD_VERSION >= OT_THREAD_VERSION_1_2
                    ((otMacFrameIsVersion2015(&p->frame)
                      && otMacFrameIsCommand(&p->frame))
                     || otMacFrameIsData(&p->frame)
                ...
                if (otMacFrameIsVersion2015(&p->frame)
                    ...
                    p->frame.mInfo.mRxInfo.mAckFrameCounter =
                        ++sMacFrameCounter;
                }
#endif // OPENTHREAD_CONFIG_THREAD_VERSION >= OT_THREAD_VERSION_1_2
                ...
                otRadio_var.dbgRxFrameNum++;
                if (otRadio_var.dbgMaxPendingFrameNum
                    < otRadio_var.dbgRxFrameNum) {
                    otRadio_var.dbgMaxPendingFrameNum =
                        otRadio_var.dbgRxFrameNum;
                }
                OT_EXIT_CRITICAL();
                // log_info("rx seq %d done", otMacFrameGetSequence(&p->frame));
                // log_info("rx done %d [%d]", otMacFrameGetSequence(&p->frame), otRadio_var.dbgRxFrameNum);
                OT_NOTIFY(OT_SYSTEM_EVENT_RADIO_RX_DONE);
            }
        }
    } else {
        // log_warn("crc error %d", crc_status);
    }
...
void rafael_radio_mac_read_config_set(uint8_t mode) { sMacAddrReadMode = mode; }
...
void ot_radioInit() {
    ...
    uint8_t ieeeAddr[OT_EXT_ADDRESS_SIZE] = {0xFF, 0xFF, 0xFF, 0xFF,
                                             0xFF, 0xFF, 0xFF, 0xFF};
    ...
    memset(&otRadio_var, 0, sizeof(otRadio_var));
    // memset(&otRadio_var, 0, offsetof(otRadio_t, buffPool));
    otRadio_var.pAckFrame = (otRadioFrame*)(otRadio_var.buffPool
                                            + TOTAL_RX_FRAME_SIZE);
    otRadio_var.pAckFrame->mPsdu = ((uint8_t*)otRadio_var.pAckFrame)
                                   + ALIGNED_RX_FRAME_SIZE;
    ...
    subg_ctrl_frequency_set(
        FREQ + (CHANNEL_SPACING * (sCurrentChannel - kMinChannel)));
    /* Auto ACK */
...
static int _cli_cmd_mac_csma(int argc, char** argv, cb_shell_out_t log_out,
                             void* pExtra) {
    ...
    do {
        ...
        /* code */
        lmac15p4_mac_pib_set(320, 544, max_be, backoffs, 16416, 4, min_be);
